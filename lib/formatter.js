...
function formatTable(elem, fn, options) {
  ...
  function tryParseRows(elem) {
    ...
    switch (elem.name.toLowerCase()) {
      ...
      case 'th':
      case 'td':
        // Sometimes tables formated without tr tag at all, so it looks like <table><td></td><table>
        // In previous logic lybrary just ignore such contect.
        _.each([{type: 'tag', name: 'tr', children: [elem]}], tryParseRows);
        return;
      case 'tr':
        var rows = [];
